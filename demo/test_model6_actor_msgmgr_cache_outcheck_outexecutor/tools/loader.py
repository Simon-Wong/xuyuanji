...
def load_tools_from_folder(files_path: Path = None)->tuple[list,dict]:
    """
    ...
    for file_path in folder_path.glob("*.py"):
        ...
        for attr_name in dir(module):
            ...
            if isinstance(attr, FunctionTool):
                ...
                if hasattr(attr, "__wrapped__") and callable(attr.__wrapped__):
                    tool_map[attr.name] = attr.__wrapped__
                else:
                    # 若无法提取，则回退到对象本身（可能不可调用）
                    tool_map[attr.name] = None
                    print(f"警告: {attr.name} 没有可调用的 __wrapped__")
